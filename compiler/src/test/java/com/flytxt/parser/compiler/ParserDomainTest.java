...
public class ParserDomainTest {
 ...
 // @Test
    ...
    public void compileNtest() throws Exception {
        String name, init, absProcessor, extract, transformation = ";", store, type, sampleData, output = null, markerSize, initMarkers;
        name = "TestJavaCode";
        init = "this.mf.setMaxListSize(7);";
        absProcessor = "protected Marker mzxcswewq;protected byte[] token_124= \"|\".getBytes();private Store store = new ConsoleStore(\"/tmp/output\");public final String folderName=\"/tmp/java/INRecharge/\";public final String regex=\"(.*)GJC(.*)\";";
        ...
        store = "store.save(data, currentFileName , mzxcswewq);";
        transformation = ";";
        markerSize = "6";
        initMarkers = "mzxcswewq=mf.create(0,0);";
        CompileNTest cNt = new CompileNTest(name, init, extract, transformation, store, type, sampleData, initMarkers, markerSize);
