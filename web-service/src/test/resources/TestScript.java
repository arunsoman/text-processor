...
private Store localfileStore = new  ConsoleStore(outputFolder,"MSISDN_FINAL","OFFNET_FINAL_MOU","OFFNET_REV_FINAL","FINAL_DATE","ONNET_FINAL_REV","ONNET_FINAL_REV");
...
mOFFNET_REV_FINAL_Metric_name= mf.createMarker(mOFFNET_REV_FINAL_Metric_name_b,0,mOFFNET_REV_FINAL_Metric_name_b.length);

byte [] mOFFNET_REV_FINAL_Metric_day_b = ("1").getBytes();

mOFFNET_REV_FINAL_Metric_day= mf.createMarker(mOFFNET_REV_FINAL_Metric_day_b,0,mOFFNET_REV_FINAL_Metric_day_b.length);

byte [] mOFFNET_REV_FINAL_Metric_month_b = ("0").getBytes();

mOFFNET_REV_FINAL_Metric_month= mf.createMarker(mOFFNET_REV_FINAL_Metric_month_b,0,mOFFNET_REV_FINAL_Metric_month_b.length);

byte [] mOFFNET_REV_FINAL_Metric_week_b = ("0").getBytes();

mOFFNET_REV_FINAL_Metric_week= mf.createMarker(mOFFNET_REV_FINAL_Metric_week_b,0,mOFFNET_REV_FINAL_Metric_week_b.length);

byte [] mOFFNET_REV_FINAL_Metric_opType_b = ("1").getBytes();

mOFFNET_REV_FINAL_Metric_opType= mf.createMarker(mOFFNET_REV_FINAL_Metric_opType_b,0,mOFFNET_REV_FINAL_Metric_opType_b.length);

byte [] mOFFNET_FINAL_MOU_Metric_name_b = ("641").getBytes();

mOFFNET_FINAL_MOU_Metric_name= mf.createMarker(mOFFNET_FINAL_MOU_Metric_name_b,0,mOFFNET_FINAL_MOU_Metric_name_b.length);

byte [] mOFFNET_FINAL_MOU_Metric_day_b = ("0").getBytes();

mOFFNET_FINAL_MOU_Metric_day= mf.createMarker(mOFFNET_FINAL_MOU_Metric_day_b,0,mOFFNET_FINAL_MOU_Metric_day_b.length);

byte [] mOFFNET_FINAL_MOU_Metric_month_b = ("0").getBytes();

mOFFNET_FINAL_MOU_Metric_month= mf.createMarker(mOFFNET_FINAL_MOU_Metric_month_b,0,mOFFNET_FINAL_MOU_Metric_month_b.length);

byte [] mOFFNET_FINAL_MOU_Metric_week_b = ("0").getBytes();

mOFFNET_FINAL_MOU_Metric_week= mf.createMarker(mOFFNET_FINAL_MOU_Metric_week_b,0,mOFFNET_FINAL_MOU_Metric_week_b.length);

byte [] mOFFNET_FINAL_MOU_Metric_opType_b = ("1").getBytes();

mOFFNET_FINAL_MOU_Metric_opType= mf.createMarker(mOFFNET_FINAL_MOU_Metric_opType_b,0,mOFFNET_FINAL_MOU_Metric_opType_b.length);
...
sb.append(neonStore.done()).append(",");
	...
	public void translate(final byte[] data){
		  if ((tpString.startsWith(mmsisdn, mMSISDN_CHECK)
  ) || (tpString.startsWith(mmsisdn, mMSISDN_CHECK2)
  )) {
    mMSISDN_FINAL = mmsisdn;mFINAL_DATE = mdate;if (tpMath.eq(mservicetype, mOFFNET, mf)
    ) {
      mOFFNET_FINAL_MOU = mMOU;mOFFNET_REV_FINAL = mREV;} else if (tpMath.eq(mservicetype, mONNET, mf)
    ) {
      mONNET_FINAL_MOU = mMOU;mONNET_FINAL_REV = mREV;}
  }
  try{mdate = tpDate.convertDate(mdate,mf,"dd-mm-yyyy");
  }catch(java.text.ParseException e){}
  try{mFINAL_DATE = tpDate.convertDate(mFINAL_DATE,mf,"dd-mm-yyyy");
  }catch(java.text.ParseException e){}

	}
	
	public void store(final byte[] data) throws IOException {
		streamStore.save(data,"",mnull,mnull,mRecharge_at_,mRecharge_dt_,mRecharge10,mOFFNET_FINAL_MOU,mnull,mFINAL_DATE,mlastModifiedTime,mpickedTime,mRecharge);

localfileStore.save(data, fileName.toString(), mMSISDN_FINAL,mOFFNET_FINAL_MOU,mOFFNET_REV_FINAL,mFINAL_DATE,mONNET_FINAL_REV,mONNET_FINAL_REV);
neonStore.save(data, fileName.toString(),mMSISDN_FINAL,mOFFNET_REV_FINAL_Metric_name,mOFFNET_REV_FINAL_Metric_day,mOFFNET_REV_FINAL_Metric_month,mOFFNET_REV_FINAL_Metric_week,mOFFNET_REV_FINAL_Metric_opType,mdate,mOFFNET_REV_FINAL);
neonStore.save(data, fileName.toString(),mMSISDN_FINAL,mOFFNET_FINAL_MOU_Metric_name,mOFFNET_FINAL_MOU_Metric_day,mOFFNET_FINAL_MOU_Metric_month,mOFFNET_FINAL_MOU_Metric_week,mOFFNET_FINAL_MOU_Metric_opType,mdate,mOFFNET_FINAL_MOU);
