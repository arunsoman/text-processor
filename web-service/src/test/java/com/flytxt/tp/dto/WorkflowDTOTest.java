...
public class WorkflowDTOTest {
	...
	private Workflow createFromString(){
		...
		String absProcessor="private Marker mtemp1;private Marker m__temp2;private byte[] token_124= \"|\".getBytes();private Marker m__temp6;private byte[] token_49= \"1\".getBytes();private boolean mtemp3;private boolean m__temp5;private Store streamStore = new StreamStore(\"/tmp/test\");private Marker m__temp4;private Marker statefull = TpConstant.booleanFalseMarker;private Marker aggregatable = TpConstant.booleanTrueMarker;private Marker m__temp__1;private Marker m;private Marker passthrough = TpConstant.booleanFalseMarker;private Marker eventType = mf.createImmutable(\"AGG_AND_TIME\".getBytes(), 0, 11);private Marker fieldType = mf.createImmutable(\"SECONDS\".getBytes(), 0, 6);private Store store = new ConsoleStore(\"__temp6\",\"temp3\");\n";
		...
		String store="streamStore.save(data, \"\", eventType, fieldType, m__temp4, statefull, aggregatable, m__temp__1, m, passthrough);store.save(data, fileName.toString(), m__temp6, mtemp3 ? TpConstant.booleanTrueMarker : TpConstant.booleanFalseMarker);\n";
		String type="single";
		...
		String inputFolder = "";
		String done = "";
		Workflow flow = new Workflow(name, init, absProcessor, extract, done, transformation, store, type, sample, outputfolder, regex, inputFolder);
		return flow;
	}
}
